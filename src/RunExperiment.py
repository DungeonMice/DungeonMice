...
def load_config(folder: str) -> dict:
    ...
    Returns:
        Diccionario ``{nombre_video: start_time}`` o ``{}`` si no existe
        el archivo.
    """
    config = {}
...
def process_single_video(video_path: str, labyrinth) -> tuple:
    ...
    Returns:
        ...
        - ``first_frame``: primer frame del video para generar imagen de
          trayectoria, o None si no se pudo leer.
    """
    ...
    # Se pasa hitbox_size para que EventLogic pueda calcular solapamiento de área
    # cuando alguna región tenga overlap_threshold > 0.0.
...
def main(labyrinth) -> None:
    ...
    Si existe un config.txt en la carpeta, usa el start_time definido por
    video. Si no, usa el start_time del input como default para todos.
    Al finalizar llama ``process_video()`` con los resultados de todos los
    videos.
